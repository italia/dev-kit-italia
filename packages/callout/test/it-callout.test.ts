...
import { CSS_PREFIX } from '@italia/test-config/utils/css-prefix.js';
...
describe('ItCallout', () => {
  ...
  describe('properties & attributes', () => {
    ...
    it('applies bigText to paragraphs in default slot', async () => {
      ...
      const textSize = styles.getPropertyValue(`--${CSS_PREFIX}callout-text-size`).trim();
      ...
      const defaultTextSize = updatedStyles.getPropertyValue(`--${CSS_PREFIX}callout-text-size`).trim();
