...
- Tooltip (in lavorazione) e [Popover](/docs/componenti-popover--documentazione): per spiegare una funzione specifica o fornire informazioni aggiuntive, sono meno invasive di una finestra modale e si possono attivare in diversi modi.
...
<div class="callout callout-primary">
  <div class="callout-inner">
    <div class="callout-title">
      <span class="text">Attivazione via Javascript</span>
    </div>
    <p>
    ...
    Al fine di garantire la completa compatibilità con tutti i moderni framework Javascript,
      <strong>dovrai implementare autonomamente la chiusura della modale se vuoi inserire dei pulsanti di chiusura all'interno dello slot `footer`.</strong>
    </p>
    <p>
      Tutti gli esempi seguenti forniscono un'implementazione di base per la chiusura della modale, analoga alla seguente, attraverso event listener sul click dei pulsanti appartenenti allo slot `footer` dei vari esempi:
    ...
    <p>
      Se stai usando un framework moderno come React, Vue o Svelte, segui le best practice per l'implementazione dei Web Component all'interno del tuo framework, ad esempio gestendo le referenze agli elementi per l'attivazione manuale della modale via JS.
    </p>
    <p>
      Vedi la sezione <strong>"Proprietà, Metodi, ed Eventi accessibili via JS"</strong> per maggiori dettagli sui metodi e gli eventi esposti dal componente.
    </p>
...
Entrambi gli eventi sono **cancelable**: il comportamento di default (apertura/chiusura) viene eseguito solo se l'evento non viene annullato. Chiamando `event.preventDefault()` puoi intercettare l'azione e, ad esempio, chiedere conferma prima di chiudere, eseguendo poi `hide()` quando opportuno:

```js
const modal = document.querySelector('it-modal');
modal.addEventListener('it-modal-close', (e) => {
  e.preventDefault(); // impedisce la chiusura di default
  if (confirm('Vuoi davvero chiudere?')) modal.hide();
});
```
