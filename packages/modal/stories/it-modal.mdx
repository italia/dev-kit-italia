...
Entrambi gli eventi sono **cancelable**: il comportamento di default (apertura/chiusura) viene eseguito solo se l'evento non viene annullato. Chiamando `event.preventDefault()` puoi intercettare l'azione e, ad esempio, eseguire logica personalizzata e richiamare poi `hide()` per chiudere la modale programmaticamente:
...
<Canvas of={Stories.ChiusuraConConferma} />
