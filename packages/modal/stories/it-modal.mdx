...
- Popola lo slot `trigger` esclusivamente con elementi interattivi semanticamente corretti, come `it-button`, `button` o che supportino il ruolo di button.
...
-  `show()`: apre la modale
```js
// Esempio di utilizzo del metodo show() per aprire la modale
...
```js
// Esempio di utilizzo del metodo hide() per chiudere la modale
...
```
-  `toggle()`:  alterna lo stato tra aperto e chiuso
```js
// Esempio di utilizzo del metodo toggle() per commutare lo stato della modale
