...
-  [Select](/docs/componenti-form-select--documentazione): per selezionare un'opzione da una lista predefinita di valori.
- [Autocomplete](/docs/componenti-form-autocomplete--documentazione): per selezionare un'opzione da un elenco di scelte più ampio, con la possibilità di filtrare le opzioni digitando.
...
> **Attenzione:** `role="menu"` non è adatto a menu di navigazione o a menu che contengono link a pagine. Non usarlo all'interno di un elemento `<nav>`.
...
Puoi personalizzare le voci del menu che viene aperto al click.
...
Usa `it-role="menu"` quando il dropdown espone comandi o azioni applicative (non link di navigazione).
In questo caso il componente assegna automaticamente `role="menuitem"` agli elementi interni.

<Canvas of={Stories.MenuAzioni} />
