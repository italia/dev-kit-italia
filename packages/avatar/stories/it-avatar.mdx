...
<description>Componente per rappresentare un utente tramite immagine, iniziali, icona o stato.</description>
...
## Accessibilità
L'avatar implementa le best practice di accessibilità:
- Supporta attributi ARIA per una migliore accessibilità
...
Per migliorare l'esperienza d'uso con i lettori di schermo,  negli avatar con icona inserisci sempre un testo alternativo: `<span class="visually-hidden">Testo icona</span>`
...
Puoi personalizzare completamente l'indicatore di presenza usando il **`slot="presence"`**:
...
Puoi personalizzare completamente l'indicatore di stato usando il **`slot="status"`**:
