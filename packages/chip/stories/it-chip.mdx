...
- un **pulsante di chiusura** nello slot `dismiss-button`, per le chip cancellabili/rimuovibili (la rimozione è gestita di default dal componente e può essere sovrascritta)
...
La proprietà `dismissable` **non aggiunge automaticamente il pulsante**: dovrai fornire un `<it-button>` con `slot="dismiss-button"`.

L'icona di chiusura deve avere dimensione `sm` per rispettare le proporzioni.

Per abilitare una chip rimuovibile è quindi sufficiente:

1. Impostare l'attributo `dismissable` sulla chip
2. Inserire un `<it-button>` nello slot `dismiss-button`

Il componente **gestisce di default la rimozione**: al click sul pulsante (anche da tastiera con `Enter` o `Spazio`, tramite il pulsante nativo) la chip emette l'evento `it-chip-close` e, se non annullato, viene rimossa dal DOM, spostando il focus su una chip adiacente quando presente.
<Canvas of={ChipStories.ChipConChiusura} />

### Logica di rimozione personalizzata

L'evento `it-chip-close` è **cancelable**. Chiamando `event.preventDefault()` puoi intercettare la rimozione ed eseguire la tua logica (ad esempio una conferma), richiamando poi il metodo pubblico `close()` quando opportuno:

```js
const chip = document.querySelector('it-chip');
...
Clicca sul pulsante di chiusura in questo esempio: la rimozione avviene solo dopo la conferma.

<Canvas of={ChipStories.ChiusuraConLogicaPersonalizzata} />
