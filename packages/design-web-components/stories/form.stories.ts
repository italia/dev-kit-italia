...
Di base, il componente \`<it-input>\` fornisce la validazione standard del browser a seconda del tipo di campo e/o degli attributi \`minlength\`, \`pattern\` e \`required\`.

In alternativa, è possibile eseguire una propria validazione (lato server, o con js), impostando l' attributo \`custom-validation="true"\`. In questo modo la validazione di defautl effettuata internamente al componente è disabilitata.
