...
Per disabilitare un elemento di paginazione (ad esempio per una pagina temporaneamente non raggiungibile), aggiungi l'attributo `disabled` a `it-pagination-item`. L'elemento assume uno stato visivo disabilitato, non è più interattivo e viene aggiunto l'attributo `aria-disabled="true"` al link al suo interno.

<Canvas of={PaginationStories.ConElementoDisabilitato} />
