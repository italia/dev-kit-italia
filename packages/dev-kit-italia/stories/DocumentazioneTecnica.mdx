...
<div class="mb-4">
  <h1>Documentazione tecnica del progetto Dev Kit Italia</h1>
  <p class="lead mb-5">
    In questa sezione vengono descritte le scelte tecniche e le linee guida adottate per la realizzazione della libreria
...
- **Avvio rapido con best practice consolidate**: include generatori di progetto (`npm @open-wc`) che integrano configurazioni moderne (es. Vite, TypeScript, testing).
- **Testing integrato**: supporta `@web/test-runner`, `sinon`, `chai` e strumenti di mocking mirati per ambienti DOM.
...
Tuttavia, in alcuni casi specifici (soprattutto legati all'accessibilità), può essere utilizzato il Light
DOM.
...
**Linea guida:**
Usare **Shadow DOM** di default. Usare **Light DOM** solo nei casi in cui ci siano impedimenti
dimostrabili alla piena accessibilità con lo Shadow DOM.
...
I Web Components verranno usati in modalità HTML-first, sfruttando gli slot e componendo tutto
il markup esternamente.
...
**NOTA: Nella cartella `examples` della root del progetto, sono forniti e documentati per gli utilizzatori finali gli snippet HTML da utilizzare sia in ambiente JS vanilla, che nei framework React, Angular, Vue, Svelte **
...
<h2>↕️ Mitigazione dei problemi di Layout Shift (CLS) e FOUC (Flash Of Unstyled Content)</h2>
