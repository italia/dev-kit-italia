...
L'internazionalizzazione dei componenti si basa sul package indipendente @italia/i18n. L'unica lingua fornita di default è l'italiano.<br/>
...
È inoltre possibile cambiare la lingua solo per una parte dei componenti presenti in pagina, passando l'attributo `lang` ad ogni singolo componente:
...
Ogni componente che prevede traduzioni, registra le proprie traduzioni tramite `registerTranslation`:
...
Se si vuole definire e localizzare un proprio componente, e abilitare su di esso le traduzioni, questo deve estendere `BaseLocalizedComponent` di `@italia/globals`:
...
La libreria di internazionalizzazione è già inclusa nel bundle globale `@italia/dev-kit-italia`. Pertanto sarà sufficiente fare:
...
Se vuoi importare solamente alcuni componenti nella tua applicazione, puoi fare:
