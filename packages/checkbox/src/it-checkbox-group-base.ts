...
export class ItCheckboxGroupBase extends FormControl {
  ...
  protected override readonly formControlController: FormControlController = new FormControlController(this, {
    ...
    value: (control: FormControl) =>
      (control as ItCheckboxGroupBase).checkboxes.filter((cb) => cb.checked).map((cb) => cb.value || 'true'),
    // Il name è quello del gruppo
    name: (control: FormControl) => (control as ItCheckboxGroupBase).name,
    // La validità è gestita da noi con checkValidity di gruppo
    checkValidity: (control: FormControl) => (control as ItCheckboxGroupBase).checkValidity(),
    reportValidity: (control: FormControl) => (control as ItCheckboxGroupBase).reportValidity(),
