...
export class ItCheckboxBase extends FormControl {
  ...
  protected readonly formControlController: FormControlController = new FormControlController(this, {
    value: (control: FormControl) =>
      (control as ItCheckboxBase).checked ? (control as ItCheckboxBase).value || 'true' : undefined,
    setValue: (control: FormControl, value: unknown) => {
      const checkbox = control as ItCheckboxBase;
  ...
  protected _renderInput(supportTextId: string, invalid: boolean, validityMessage: string) {
  ...
  protected getSupportTextRender(supportTextId: string) {
    return html` ${when(
      this.supportText,
  ...
  protected getValidityMessageRender(validityMessage: string) {
    return html`<div
      role="alert"
