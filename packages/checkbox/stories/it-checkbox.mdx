...
## Gruppo di checkbox
Per creare un gruppo di checkbox, racchiudi tutti gli elementi `<it-checkbox>` all'interno del componente `<it-checkbox-group>` e assegna al gruppo l'attributo `name` corrispondente al nome del campo da salvare nel form.

Sul componente `<it-checkbox-group>` sono disponibili alcuni attributi che si riflettono automaticamente sugli `<it-checkbox>` figli, senza doverli replicare in ognuno di essi.
...
- `group`, applica lo stile di default *gruppo di checkbox*;
...
È lo stile di default del gruppo di checkbox ed equivale a impostare l'attributo `group` su `<it-checkbox-group>`.
