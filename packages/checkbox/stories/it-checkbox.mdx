...
## Gruppo di checkbox
Per creare un gruppo di checkbox, racchiudi tutti gli element `<it-checkbox>` all'interno del componente `<it-checkbox-group>` e assegna al gruppo l'attributo `name` corrispondente al nome del campo da salvare nella form. Non è necessario ripertere l'attributo su ogni singolo checkbox figlio.
...
In questi casi, al submit della form via js, il modo corretto per estrarre il valore del campo è:
