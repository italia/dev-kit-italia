...
## Gruppo di checkbox
Per creare un gruppo di checkbox, racchiudi tutti gli element `<it-checkbox>` all'interno del componente `<it-checkbox-group>` e assegna al gruppo l'attributo `name` e `required` nel caso in cui il gruppo di checkbox sia obbligatorio.
